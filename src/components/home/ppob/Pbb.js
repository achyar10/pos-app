...
const Pbb = (props) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const hit = await fetchPost(postpaidByCode, { type: 'pbb' })
                setData(hit.data)
            } catch (error) {
            ...
            <Modal show={props.show} onHide={props.close} backdrop="static" keyboard={false} size='md' animation={false} className={modal && 'hide'}>
                ...
                <Modal.Body>
                    ...
                    <div className="form-group">
                        ...
                        <input type="text" className="form-control" placeholder="Masukan nomor objek pajak..." onChange={(e) => setHp(e.target.value)} />
                    </div>
