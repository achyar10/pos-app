...
const Finance = (props) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const hit = await fetchPost(postpaidByCode, { type: 'finance' })
                setData(hit.data)
            } catch (error) {
    ...
    const handlePay = async () => {
        try {
            ...
            if (hit.status) {
                Alert('Pembayaran Angsuran/Kredit Berhasil')
                handleClose()
                setObj(null)
            } else {
                Alert(hit.message)
            ...
            <Modal show={props.show} onHide={props.close} backdrop="static" keyboard={false} size='md' animation={false} className={modal && 'hide'}>
                ...
                <Modal.Body>
                    ...
                    <div className="form-group">
                        ...
                        <input type="text" className="form-control" placeholder="Masukan nomor Kontrak..." onChange={(e) => setHp(e.target.value)} />
                    </div>
